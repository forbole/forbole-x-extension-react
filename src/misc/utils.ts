  ...
  ;(coins || []).forEach((coin) => {
    ...
    denomsToUse.some((d) => {
      const unit = get(d, 'token_unit.token.token_units', []).find(
        (t: { denom: string }) => t && coin && t.denom === coin.denom
      )
      if (unit) {
        const base = get(d, 'token_unit.token.token_units', []).find((t: { denom: string }) => t.denom === d.unit_name)
...
export const transformGqlAcountBalance = (data: any, timestamp: number): AccountBalance => {
  ...
  const balance = {
    ...
    delegated: getTokenAmountFromDenoms(
      get(data, 'account[0].delegated', []).map((d: { amount: any }) => d.amount),
      denoms
    ),
    unbonding: getTokenAmountFromDenoms(
      get(data, 'account[0].unbonding', [])
        .filter((d: { completion_timestamp: string | number | Date }) => new Date(d.completion_timestamp).getTime() > Date.now())
        .map((d: { amount: any }) => d.amount),
      denoms
    ),
    rewards: getTokenAmountFromDenoms(
      get(data, 'account[0].rewards', []).map((d: any) => get(d, 'amount[0]')),
      denoms
    ),
    commissions: getTokenAmountFromDenoms(
      get(data, 'account[0].validator.validator.commissions', []).map((d: { amount: any }) => d.amount),
      denoms
    ),
  }
  return {
...
export const transformValidatorsWithTokenAmount = (data: any, balanceData: any) => {
  ...
  const delegatedByValidator:any = {}
  get(balanceData, 'account[0].delegated', []).forEach((d: { amount: { denom: string; amount: string } }) => {
    delegatedByValidator[get(d, 'validator.validator_info.operator_address', '')] =
      getTokenAmountFromDenoms([d.amount], tokensPrices)
  })
  const rewardsByValidator:any = {}
  get(balanceData, 'account[0].rewards', []).forEach((d: { amount: { denom: string; amount: string }[] }) => {
    rewardsByValidator[get(d, 'validator.validator_info.operator_address', '')] =
      getTokenAmountFromDenoms(d.amount, tokensPrices)
  })
  const unbondingByValidator:any = {}
  get(balanceData, 'account[0].unbonding', []).forEach((d: { completion_timestamp: string | number | Date; amount: { denom: string; amount: string } }) => {
...
export const transformUnbonding = (data: any, balanceData: any): Unbonding[] => {
  ...
  return get(balanceData, 'account[0].unbonding', [])
    .map((u: { amount: { denom: string; amount: string }; height: any; completion_timestamp: string | number | Date }) => ({
    ...
    .filter((r: { completionDate: { getTime: () => number } }) => r.completionDate.getTime() > Date.now())
    ...
    .map((t: { type: string | string[] }) => {
      ...
      if (t.type.includes('MsgMultiSend')) {
        return {
          ...
          detail: {
            inputs: get(t, 'value.inputs', []).map((input: { coins: { denom: string; amount: string }[] }) => ({
              ...input,
              tokenAmount: getTokenAmountFromDenoms(input.coins, tokensPrices),
            })),
            outputs: get(t, 'value.outputs', []).map((output: { coins: { denom: string; amount: string }[] }) => ({
              ...output,
...
export const getEquivalentCoinToSend = (
  ...
  const coinDenom: { denom: string; exponent: number } = get(
    ...
    []
  ).find((unit: { denom: string }) => !!availableCoins.find((c) => c.denom.toLowerCase() === unit.denom.toLowerCase()))
  return {
    amount: Math.round(
...
export const transformProposal = (
  ...
  return {
    ...
    tag: getTag(get(p, 'status')),
    duration: differenceInDays(new Date(get(p, 'voting_end_time')), Date.now()),
    depositDetails: get(p, 'proposal_deposits', []).map((x: any) => {
...
export const transformVoteSummary = (proposalResult: any): any => {
  ...
  get(proposalResult, 'proposal_tally_result', []).forEach((p: any) => {
...
export const transformVoteDetail = (voteDetail: any, proposal: Proposal): any => {
  return get(voteDetail, 'proposal_vote', []).map((d: any) => {
...
export const transformChainConnections = (data: any): ChainConnection[] =>
  get(data, 'chain_link', []).map((d: { creation_time: string | number | Date; external_address: string; user_address: any }) => ({
...
export const isValidMnemonic = (input: string) => {
  try {
